...
## 8. Notification policy (owner preference, 2026-09-13)
Hermes stays QUIET while working — mid-task progress asides ("lemme pull that up…") are reserved for security scans/audits and findings reports only (MAX_NOTIFIES=1, scan/report-only instruction in the system prompt). Ordinary tool calls produce no chatter, just the final answer. Don't revert this without asking Ace.
